...
namespace MonkeyIslandSpecialEditionSpriteEditor.Commands
{
	public class ApplyOverrideCommand( string? lpakFilePath, string? resourcePath ) : BaseCommand
	{
		protected override CommandResult InnerExecute()
		{
			...
			if( isCostume )
			{
				entity = MonkeyIslandSpecialEditionSpriteEditor.Formats.Costumes.Parser.ReadCostumeFromXmlFile( overrideXmlPath );
			} else if( isRoom )
			{
				entity = MonkeyIslandSpecialEditionSpriteEditor.Formats.Rooms.Parser.ReadRoomFromXmlFile( overrideXmlPath );
			}
			...
			try
			{
				if( isCostume )
				{
					MonkeyIslandSpecialEditionSpriteEditor.Formats.Costumes.SanityChecker.Check( (entity as Costume)! );
				}
				else if( isRoom )
				{
					MonkeyIslandSpecialEditionSpriteEditor.Formats.Rooms.SanityChecker.Check( (entity as Room)! );
				}
			}
			...
			if( isCostume )
			{
				MonkeyIslandSpecialEditionSpriteEditor.Formats.Costumes.Packer.WriteCostumeToBinaryFile( binaryResourcePath, (entity as Costume)! );				
			} else if( isRoom )
			{
				MonkeyIslandSpecialEditionSpriteEditor.Formats.Rooms.Packer.WriteRoomToBinaryFile( binaryResourcePath, (entity as Room)! );
			}
